...
const analyzeEmotion = async (audio: Blob | File): Promise<{ emotion: string; confidence: number }> => {
  ...
  const response = await fetch('http://localhost:3000/findAccuracy', {
    method: 'POST',
    body: formData,
  });
  ...
  const result = await response.json(); // Store the result once
  console.log(result); // Log it
  return result; // Return the parsed result // Assuming the backend returns { emotion: string, confidence: number }
};
